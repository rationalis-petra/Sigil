...
canon_lookup lift_err (Name name) canon = case name of
  Left (Path (package_name, mpath)) -> case Map.lookup package_name (global_env canon) of 
...
canon_insert lift_err (Name name) (mval, ty) canon = case name of 
  Left _ -> throwError $ lift_err $ "Can't insert variable at path, only as local name"
  Right unique -> do
    let val' = maybe (Var (Name name)) id mval
        sem_env = (fmap (\(x,_,_) -> x) (local_env canon), fmap (\(x,_) -> x) (global_env canon))
        
    sem <- let ?lift_err = lift_err in eval val' sem_env
    pure $ CanonEnv (global_env canon) (Map.insert unique (sem, ty, val') (local_env canon))


-- to_abs_env :: CanonEnv m -> Env (CanonEnv m) m

to_semenv :: CanonEnv m -> SemEnv m
to_semenv canon = (fmap (\(x,_,_) -> x) (local_env canon) , fmap fst (global_env canon))
