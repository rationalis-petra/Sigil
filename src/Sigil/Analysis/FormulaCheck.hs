...
-- TODO: update check_formula to generate a formula for solving...
...
check_formula interp@(CheckInterp {..}) env formula = case formula of 
  ...
  Bind q n ty f -> do
    (ty', kind) <- infer_core interp env ty
    ty_norm <- normalize (lift_err . NormErr mempty) (i_impl env) kind ty'
...
check_scons interp@(CheckInterp {..}) env cons = case cons of
  (l :≗: r) -> do
    (l', lty) <- infer_core interp env l
    (r', rty) <- infer_core interp env r
    n <- get_universe lift_err (range l <> range r) env lty
    check_eq mempty interp env (Uni n) lty rty
    l_norm <- normalize (lift_err . NormErr mempty) (i_impl env) lty l'
    r_norm <- normalize (lift_err . NormErr mempty) (i_impl env) rty r'
    pure $ (l_norm :≗: r_norm)
  (val :∈: ty) -> do
    (ty', kind) <- infer_core interp env ty
    ty_norm <- normalize (lift_err . NormErr mempty) (i_impl env) kind ty'
    val' <- check_core interp env val ty_norm
    val_norm <- normalize (lift_err . NormErr mempty) (i_impl env) ty_norm val'
